...
plot(mco.wwf.pred.r, main="Mid-Miocene Climatic Optimum, WWF Predictions", col=biome.col2)
```
...
plot(mce.wwf.pred.r, main="Late Miocene Cooling Event, WWF Prediction", col=biome.col2)
```
...
plot(pwe.wwf.pred.r, main="Pliocene Warming Event, WWF Predictions", col=biome.col2)
```
...
plot(pir.wwf.pred.r, main="Pre Industrial Revolution, WWF Predictions", col=biome.col2)
```
...
Biome_change <- function(geoP1, geoP2, biomes=c("Cold","Dry","Wet","Xeric")){
  ...
  qgraph(df_tmp[,c(2:3,1)])
}

Biome_change(pew.wwf.pred.r,tee.wwf.pred.r)

```

```{r}
# Check qgraph R package for the main transition graphics. 
# http://sachaepskamp.com/qgraph/examples (check the UNWEIGHTED DIRECTED GRAPHS)
tmp2 <- tmp %>% 
  filter(period=="pew_tee") %>% 
  dplyr::select(from,to,count)
  
nm<-matrix(NA, nrow=4, ncol=4)
colnames(nm) <- c("Cold","Dry","Wet","Xeric")
rownames(nm) <- c("Cold","Dry","Wet","Xeric")
diag(nm)<-1

nm[as.matrix(tmp2[, 1:2])] <- tmp2[,3]
...
pdf("./figs/Biome_change.pdf")
ggplot(biomes_pred2, aes(x=Period, y=Prop, group=biomes)) +
  geom_line(aes(color=biomes),size=2)+
